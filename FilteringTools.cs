...
namespace Filtering
{
    ...
    class FilteringTools
    {
        ...
        /// </remarks>
        public static double[] FiltFilt(double[] data, double[] a, double[] b)
        {
            // Find the filter initial conditions
            double[] zi = FilteringTools.initialStateFilter(b, a);
        ...
        public static double[] initialStateFilter(double[] b, double[] a)
        {
            // Define the initial filter state (according to Matlab filtfilt function)
            int lengthB = b.Length;
            int lengthA = a.Length;
            int nFilt = Math.Max(lengthB, lengthA);
            //int L = 1; // Viene definito in Matlab ma non mi serve
            double[] b1Temp = b.Select(r => r * a[0]).ToArray();
            double[] a1Temp = a.Select(r => r * a[0]).ToArray();
            ...
            // Zero pad shorter coefficient vector if needed
            if (lengthB == nFilt)
            {
                b1 = b1Temp;
            }
            ...
            // Obtain zi parameters (initial state of the filter)
            if (nFilt > 1)
            {
                // Costruisco le matrici A e B che definiscono il sistema lineare risolto dal vettore zi (condizioni iniziali)
                var A = Matrix<double>.Build.DenseOfArray(new double[nFilt - 1, nFilt - 1]);
                var bVector = MathNet.Numerics.LinearAlgebra.Vector<double>.Build.Dense(new double[nFilt - 1]);
                ...
                for (int i = 0; i < nFilt - 1; i++)
                {
                    ...
                    bVector[i] = b1[i + 1] - b1[0] * a1[i + 1];
                }
                var ziTemp = A.Solve(bVector);
